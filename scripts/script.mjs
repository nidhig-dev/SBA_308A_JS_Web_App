...
async function handleGetBreedInfo(event) {
    try {
        ...
        // Breed Group: ${res.data['breed_group']} 
        // Weight: ${res.data.weight.imperial} pounds
        // Height: ${res.data.height.imperial} inches 
        // Temperament: ${res.data.temperament} 
        // Bred-for: ${bredFor};
        // Life-Span: ${res.data['life_span']} `;
...
async function getFavourites(event){
  ...
  for(let i=0;i<existRes.data.length;i++){
    ...
    console.log("image id is",existRes.data[i].image_id);
      ...
      let img = Carousel.createCarouselItem(existRes.data[i].image.url, "", existRes.data[i].image_id)
        Carousel.appendCarousel(img);
        // Carousel.start();
    }
    }
